...
menyisipkan tangkapan layar pada Bab 11 (Tampilan Situs).

Tangkapan layarnya dihasilkan lebih dulu oleh tools/capture-screens.mjs.
...
OUTPUT = os.path.join(ROOT, 'docs', 'Laporan_TPD_WEBSEKOLAH.pdf')

PAGE_W, PAGE_H = A4
MARGIN = 18 * mm
CONTENT_W = PAGE_W - 2 * MARGIN

BLUE = colors.HexColor('#112c55')
DARK = colors.HexColor('#0b1e3b')
GREY = colors.HexColor('#6b7688')
LINE = colors.HexColor('#dbe0e8')
CODE_BG = colors.HexColor('#f6f8fa')
HEAD_BG = colors.HexColor('#e2ebf7')
...
FIGURES = [
    ('Halaman Situs/1 Beranda.png',
     'Halaman 1 — Beranda (/). Menu utama tembus pandang di atas gambar sampul Swiper, tagline sekolah, dan tiga angka pokok.'),
    ('Halaman Situs/2 Profil Sekolah.png',
     'Halaman 2 — Profil Sekolah (/profil). Visi dan misi sekolah, dilanjutkan sambutan kepala sekolah, sejarah, sarana, serta Tabel Informasi Profil Sekolah.'),
    ('Halaman Situs/3 Program Keahlian.png',
     'Halaman 3 — Program Keahlian (/program-keahlian). Tiga program beserta kompetensi yang dipelajari dan prospek lulusannya.'),
    ('Halaman Situs/4 Ekstrakurikuler.png',
     'Halaman 4 — Ekstrakurikuler (/ekstrakurikuler). Enam kegiatan lengkap dengan jadwal dan pembina, dapat disaring per kategori.'),
    ('Halaman Situs/5 Galeri.png',
     'Halaman 5 — Galeri (/galeri). Sembilan dokumentasi kegiatan dengan penyaring kategori beserta jumlah foto tiap kategori.'),
    ('Halaman Situs/6 Berita.png',
     'Halaman 6 — Berita (/berita). Enam berita diurutkan dari yang terbaru, dengan kotak pencarian yang menyaring saat diketik.'),
    ('Halaman Situs/7 Detail Berita.png',
     'Halaman 7 — Detail Berita (/berita/juara-lks-otomotif). Dibangun otomatis satu halaman untuk setiap berita di dalam berkas data.'),
    ('Halaman Situs/8 Kontak.png',
     'Halaman 8 — Kontak (/kontak). Formulir bervalidasi, alamat, jam layanan, dan daftar pertanyaan yang sering diajukan.'),
    ('Halaman Situs/9 Halaman Tidak Ditemukan.png',
     'Halaman galat 404 berbahasa Indonesia beserta tautan kembali ke menu utama, tanpa memunculkan pesan teknis.'),
    ('Bukti Fitur/Tabel Informasi Profil Sekolah.png',
     'Ketentuan soal nomor 5 — Tabel Informasi Profil Sekolah berisi 22 baris, dibangun lewat perulangan atas sebuah array.'),
    ('Bukti Fitur/Informasi Jumlah Guru dan Siswa.png',
     'Ketentuan soal nomor 3 — informasi jumlah guru dan siswa: 42 guru, 898 peserta didik, 27 rombongan belajar, dan 24 mitra industri, beserta grafik lingkaran Chart.js yang digambar pada elemen canvas.'),
    ('Bukti Fitur/Berita Kegiatan di Beranda.png',
     'Ketentuan soal nomor 3 — berita kegiatan sekolah pada halaman utama, memakai komponen kartu yang sama dengan halaman Berita.'),
    ('Bukti Fitur/Galeri di Beranda.png',
     'Ketentuan soal nomor 3 — cuplikan galeri pada halaman utama.'),
    ('Bukti Fitur/Video Profil Sekolah.png',
     'Unit multimedia — elemen video dengan tombol putar buatan sendiri. Videonya dibuat sendiri sebagai animasi web, berdurasi 20,5 detik pada 1920x1080, 60 gambar per detik.'),
    ('Bukti Fitur/Menu Seluler.png',
     'Unit user interface — menu hamburger pada lebar layar ponsel. Isinya berasal dari array menu yang sama dengan menu layar lebar.'),
    ('Bukti Fitur/Penyaring Kategori Galeri.png',
     'Penyaring galeri pada kategori Praktik: dua foto tersisa, dan instansi lightbox dibuat ulang agar hanya berpindah di antara foto yang terlihat.'),
    ('Bukti Fitur/Lightbox Galeri.png',
     'Library GLightbox memperbesar foto tanpa berpindah halaman, lengkap dengan judul dan kategorinya.'),
    ('Bukti Fitur/Penyaring Ekstrakurikuler.png',
     'Penyaring ekstrakurikuler pada kategori Wajib: satu kegiatan tersisa.'),
    ('Bukti Fitur/Pencarian Berita.png',
     'Pencarian berita untuk kata kunci "prestasi": satu berita ditemukan, dan keterangan jumlahnya ikut diperbarui.'),
    ('Bukti Fitur/Validasi Formulir Kontak.png',
     'Validasi formulir kontak saat dikirim dalam keadaan kosong: empat pesan galat muncul, masing-masing spesifik pada isiannya.'),
    ('Bukti Fitur/Formulir Kontak Terkirim.png',
     'Formulir yang terisi benar diterima, ditandai pesan berhasil berwarna hijau, dan seluruh isian dikosongkan kembali.'),
]
            ...
            if level == 1:
                ...
                if text.startswith('11.'):
                    story.append(Paragraph(
                        'Berikut tangkapan layar seluruh halaman situs dan '
                        'bukti berjalannya setiap fitur yang telah dibangun. '
                        'Seluruh gambar diambil dari hasil build produksi.',
                        ST['body']))
...
def cover():
    ...
    st.append(Paragraph('<b>WEBSITE SEKOLAH</b>', ParagraphStyle(
        'x', parent=ST['cover-title'], fontSize=20, textColor=BLUE)))
    st.append(Paragraph('SMK Wira Teknologi Nusantara', ST['cover-sub']))
...
def main():
    ...
    doc = BaseDocTemplate(
        ...
        title='Laporan Tugas Praktik Demonstrasi - Website Sekolah',
